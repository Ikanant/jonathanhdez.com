...
jQuery(document).ready(function ($) {
	...
	function goToByScroll(dataslide) {
		...
		if(dataslide == 1){
			window.open("https://drive.google.com/file/d/0B3HoxedYLzLGQVdaemJwWDZmSXc/view?usp=sharing", "_blank");
		}
		else{
	...
	arrows.click(function(e) {
		...
		switch( $(this).attr('id') ) {
			...
			case 'arrow-down':
				...
				break;
			case 'arrow-left':
				offset_left = $('#slide-3 .horizontalScroll').offset().left + 652;
				if ( offset_left > 0 ) {
					offset_left = '0px';
				}
				break;
			case 'arrow-right':
				offset_left = $('#slide-3 .horizontalScroll').offset().left - 652;
				if ( offset_left < $('body').width() - $('#slide-3 .horizontalScroll').width() ) {
					offset_left = $('body').width() - $('#slide-3 .horizontalScroll').width();
				}
				break;
		}
		...
		if ( offset_left != false ) {
			if ( $('#slide-3 .horizontalScroll').width() != $('body').width() ) {
				$('#slide-3 .horizontalScroll').stop(false, false).animate({
					left: offset_left
				}, 1500, 'easeInOutQuart');
			}
		}
